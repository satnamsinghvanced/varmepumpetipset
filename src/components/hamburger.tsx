"use client";

import { NAV_LINKS } from "@/const/consts";
import Link from "next/link";
import { useState } from "react";
import { FaArrowRightLong } from "react-icons/fa6";

const Hamburger = () => {
  const [isOpen, setIsOpen] = useState(false);
  const toggleMenu = () => setIsOpen(!isOpen);
  return (
    <>
      <button
        onClick={toggleMenu}
        aria-label="Toggle navigation"
        className="flex flex-col justify-center items-center space-y-1.5 md:hidden cursor-pointer relative w-8 h-8"
      >
        <span className={`hamburger-line ${isOpen ? 'hamburger-line-top-open' : ''}`} />
        <span className={`hamburger-line ${isOpen ? 'hamburger-line-middle-open' : ''}`} />
        <span className={`hamburger-line ${isOpen ? 'hamburger-line-bottom-open' : ''}`} />
      </button>
      {isOpen && (
        <div className="bg-background p-4 w-full fixed top-[82px] right-0 h-full z-10">
          <ul className="list-none p-0 m-0 px-4 pr-6">
            {NAV_LINKS?.map((link) => (
              <li key={link.href} className="mb-3 group">
                <Link
                  href={link.href}
                  onClick={toggleMenu}
                  className="flex justify-between"
                >
                  <span className="text-lg font-medium text-dark/80 group-hover:translate-x-2 group-hover:text-dark transition ease-in-out duration-300">
                    {link.label}
                  </span>
                  <span>
                    <FaArrowRightLong width="28" className="text-dark" />
                  </span>
                </Link>
              </li>
            ))}
          </ul>
        </div>
      )}
    </>
  );
};

export default Hamburger;
