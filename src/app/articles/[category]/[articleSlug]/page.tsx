...
export async function generateMetadata({ params }: SlugPageProps) {
  ...
  return generatePageMetadata({
    title: metaTitle || ` | Varmepumpetipset.no`,
    description: metaDescription || `Read expert articles about ${title} on Varmepumpetipset.no.`,
    path: `articles/${articleCategory}/${canonicalUrl}`,
    keywords: metaKeywords ? metaKeywords.split(",").map((k: string) => k.trim()).filter(Boolean) : ["varmepumpetipset", "real estate", "articles"],
    type: ogType || "website",
    image: ogImage || null,
    ogTitle: ogTitle || metaTitle || `${title} | Varmepumpetipset.no`,
    ogDescription: ogDescription || metaDescription || `Explore helpful ${title} articles from Varmepumpetipset.no.`,
